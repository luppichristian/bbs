...
description: Create, explain, validate, and repair projects that use the `bbs` build system and its `.bbs` syntax. Use when Codex needs to author or edit `project.bbs`, choose valid target types and fields, emit correct `bbs build/run/test/dist/update/package/gen/info/cfg` commands, model package-backed targets or `builders(...)`, or avoid generic CMake assumptions for `bbs`-based C, C++, or CUDA projects.
---
